...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  // Assuming isbn is extracted from the route parameters
const isbn = req.params.isbn;

// Retrieve review from the request body
const userReview = req.body.review;

// Check if user is defined in the session
const user = req.session.user;


const bookToBeReviewed = Object.values(books).filter(book => book.isbn === isbn);

if (bookToBeReviewed.length === 0) {
...
return res.status(200).json({
  message: `Review: '${userReview}' was added by '${user}'`,
